...
# A source built with Source.new_with_uid() never gains a D-Bus proxy, so
# re-using it after its commit makes EDS treat the next write as a second
# create and fail with "UID is already in use". Every commit must hand back
# the registry-backed source instead.
